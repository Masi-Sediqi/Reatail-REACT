...
function FloatingActionMenu({ actions, ariaLabel, className = '', width = 172 }) {
  ...
  useEffect(() => {
    ...
    const updatePosition = () => {
      ...
      if (!rect) return
      const dir = buttonRef.current.closest('[dir="rtl"]') ? 'rtl' : 'ltr'
      ...
      const preferredLeft = dir === 'rtl' ? rect.left : rect.right - width
  ...
  return (
    <div className="row-actions">
      ...
      {open && createPortal(
        ...
        buttonRef.current?.closest('.retail-shell') ?? document.body,
      )}
    </div>
  )
}
