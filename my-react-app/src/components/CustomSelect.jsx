...
function CustomSelect({ ariaLabel, className = '', onChange, options, value }) {
  ...
  useEffect(() => {
    ...
    const updatePosition = () => {
      ...
      const top = below + menuHeight > window.innerHeight - viewportPadding && above > viewportPadding ? above : below
      const left = clamp(rect.left, viewportPadding, window.innerWidth - menuWidth - viewportPadding)
      const availableHeight = top < rect.top
  ...
  return (
    <div className={`smooth-select ${className}`.trim()} ref={rootRef}>
      <button
        ...
        type="button"
      >
        <span className="smooth-select-label">{selected?.label}</span>
        <ChevronDown className={open ? 'rotate' : ''} size={16} />
      </button>
      {open && createPortal(menu, rootRef.current?.closest('.retail-shell') ?? document.body)}
    </div>
  )
}
